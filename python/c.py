...
def processarArquivo(file_path, output_file_path):
    try:
        ...
        resultado_excel = output_file_path.replace('.csv', '.xls')  # Nome do arquivo Excel
        newIndexNoDuplicates.to_excel(resultado_excel, index=False)
...
if __name__ == "__main__":
    ...
    caminho_arquivo = r"H:\spoloar\MODELO PADRAO PARA CONCILIAÇÃO DE DEBITOS E CREDITOS DE FORNECEDORES.csv"
    caminho_saida = r"H:\spoloar\MODELO PADRAO PARA CONCILIAÇÃO DE DEBITOS E CREDITOS DE FORNECEDORES.csv11"
    processarArquivo(caminho_arquivo, caminho_saida)
